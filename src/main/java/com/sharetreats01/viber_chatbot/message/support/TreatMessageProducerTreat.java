...
public class TreatMessageProducerTreat implements TreatMessageProducer {
    ...
    public MessageRequest produceMessage(MessageProcessContext context) {
        ...
        // Rich Media
        RichMediaType richMediaType = treatMessageUtils.extractRichMediaForTreatComp(context.getTrackingData());
        ...
        if (richMediaType == RichMediaType.TREAT_COMP_FRIEND) {
            buttons = MessagePropertyUtils.ConvertToTreatFriendRichMediaButtonList(richMediaButtonEntityRepository.findAllByRichMedia_Id(richMedia.getId()), treatMessageUtils.createTreatFriendPropertyList(context.getTrackingData()));
        } else if (richMediaType == RichMediaType.TREAT_COMP_ME) {
            buttons = MessagePropertyUtils.ConvertToTreatMeRichMediaButtonList(richMediaButtonEntityRepository.findAllByRichMedia_Id(richMedia.getId()), treatMessageUtils.createTreatMePropertyList(context.getTrackingData()));
        }
